...
class KnowledgeBasesForAmazonBedrock:
    ...
    def __init__(self, suffix=None):
        ...
        self.s3_bucket_name = f"anycompany-products-dataset-{self.region_name}-{resource_suffix}"
        self.kb_name = f"anycompany-products-{resource_suffix}"
        self.kb_desc = "anycompany-products"
    ...
    def create_or_retrieve_knowledge_base(
        ...
        #data_bucket_name: str = 'anycompany-products-dataset',
        ...
        """
        data_bucket_name = self.s3_bucket_name
        kb_id = None
        ds_id = None
    ...
    def upload_directory(self, s3_path, bucket_name):
        ...
        """
        # print(s3_path)
        # print(os.listdir(s3_path))
        # for files in os.listdir(s3_path):
        #     file_to_upload = os.path.join(s3_path, files)
        #     print(f"uploading file {file_to_upload} to {bucket_name}")
        #     self.s3_client.upload_file(file_to_upload, bucket_name, files)
...
if __name__ == "__main__":
    ...
    if args.mode == "create":
        kb_id, ds_id = kb.create_or_retrieve_knowledge_base(
            kb.kb_name, kb.kb_desc
        )
        print(f"Knowledge Base ID: {kb_id}")
        print(f"Data Source ID: {ds_id}")
        kb.upload_directory(
            f'{current_dir}/data_files/productFiles', kb.get_data_bucket_name()
        )
